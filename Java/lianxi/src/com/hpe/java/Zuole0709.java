...
public class Zuole0709 {
	...
	public static void test7() {
		...
		// 方式一
		/*
		 * int sum = 0; double avg; int max = 0; int min = -1; Scanner sc = new
		 * Scanner(System.in);
		 * 
		 * for(int i = 0; i < 10; i++){
		 * 
		 * System.out.println("请输入得分："); int score = sc.nextInt();
		 * 
		 * if(i ==0){ max = score; min = score; } if(max < score ) max = score;
		 * 
		 * if(min > score) min = score;
		 * 
		 * sum += score;
		 * 
		 * } System.out.println(sum); System.out.println(max);
		 * System.out.println(min); sum = sum - max - min;
		 * 
		 * avg = sum / 8.0;
		 * 
